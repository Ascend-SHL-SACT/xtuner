...
def register_dsa_topk_offload_npu_hooks(
    *,
    ...
    - each DSA attention's `sparse_mla_func` instance attribute is wrapped so
      ...
      backward order pops the entry, later consumers are no-ops. That first
      refill also relaunches every other parked source's H2D in one batch: all
      refill blocks are allocated at a single point of the step (so the
      park/free and refill/re-allocate cycle closes instead of leaving the
      allocator a churned footprint of slack), and each later consumption only
      waits on its copy's event.
...
def _wrap_sparse_mla_with_refill(attn: DSAMultiLatentAttention, layer_idx: int) -> None:
    ...
    # and inference never parks. The first replayed consumer refills its own
    # source and relaunches every other parked source's H2D (see
    # _ensure_dsa_topk_resident); later consumers find the registry empty and
    # pass through.
...
def _ensure_dsa_topk_resident(source_layer_idx: int) -> None:
    ...
    # parked it on the CPU, and relaunch every other parked source's H2D with
    # it. Called by the sparse_mla_func wrapper right before consumption: the
    # backward replay re-enters layers in reverse order, so a source's last
    # consumer is the first replayed reader of its ids and pops the entry,
    # refilling every micro-batch; later replays of the same source find
    # nothing parked (no-op), as does every original (no_grad) forward.
    #
    # Every refill storage is re-expanded on the working (compute) stream and
    # every H2D runs on the one offload stream. Re-expanding inside the
    # offload-stream context instead strands the block in that stream's
    # allocator pool, where the compute stream can never reuse it. And
    # relaunching the refills one consumer at a time lets the intermediate
    # forward-tail/backward transients carve up the freed ids blocks, so the
    # refill re-allocations never reclaim their own freed blocks and the
    # allocator settles one churned footprint (the ids total size) above the
    # no-offload reserved level. Batching the relaunch at the first replayed
    # consumption allocates every refill block at one point of the step, with
    # an identical allocation sequence every step, so the park/free and
    # refill/re-allocate cycle closes. The source consumed now takes the
    # blocking path (its reader cannot wait); the rest launch async H2Ds that
    # their own consumption points wait on (event-only in the steady state).
    swap_tensors = _topk_offloads.pop(source_layer_idx, None)
    if swap_tensors is None:
        return

    working_stream = torch.cuda.current_stream(swap_tensors[0].tensor.device)
    stream = _topk_stream(swap_tensors[0].tensor.device)
    for swap_tensor in swap_tensors:
        if swap_tensor.stat != "host":
            # Prefetched state (self-healing paths after an aborted step):
            # only the event wait remains (launch_h2d's waiting path is a
            # no-op on an event that was never recorded, so never-evacuated
            # swaps pass through too).
            swap_tensor.launch_h2d(stream, True, working_stream)
            continue
        # Blocking H2D on the offload stream for the source being consumed
        # now, same side-stream choreography as the activation offload: the
        # copy is issued on the offload stream after it waits on the working
        # stream, and the working stream then orders itself behind the copy,
        # so the layer body reads refilled storage.
        stream.wait_stream(working_stream)
        swap_tensor.tensor.storage().resize_(swap_tensor.storage_size)
        with torch.cuda.stream(stream):
            swap_tensor.launch_h2d(stream, False, stream)
        working_stream.wait_stream(stream)

    for entry in _topk_offloads.values():
        for swap_tensor in entry:
            if swap_tensor.stat != "host":
                continue
            swap_tensor.tensor.storage().resize_(swap_tensor.storage_size)
            swap_tensor.prefetch_launch_h2d(stream, False)
